                ...
                <ul className="space-y-4 mx-3 mt-3">
                    {
                        messages && messages.map((msg,index)=>(
                            <li className={`${(msg?.senderId && user?.id && msg.senderId=== user.id.toString()) ? "flex items-start mr-2 py-2 px-4 max-w-fit bg-blue-500 rounded-bl-3xl rounded-tl-3xl rounded-tr-xl text-white" : "ml-auto justify-end py-2 px-4 max-w-fit bg-green-700 rounded-br-3xl rounded-tr-3xl rounded-tl-xl text-white"}`} key={index}>
